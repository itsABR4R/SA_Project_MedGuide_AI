...
export function loadConfig({ env = process.env, argv = process.argv } = {}) {
  ...
  const appUrl = env.OPENROUTER_APP_URL || `http://localhost:${port}`;
  const configuration = {
    env: environment,
    ...
    trustProxy: booleanValue('TRUST_PROXY', env.TRUST_PROXY, false),
    http: {
    ...
    mongodb: {
      ...
      maxPoolSize: integerValue('MONGODB_MAX_POOL_SIZE', env.MONGODB_MAX_POOL_SIZE, 10, { min: 1, max: 100 })
    },
    session: {
