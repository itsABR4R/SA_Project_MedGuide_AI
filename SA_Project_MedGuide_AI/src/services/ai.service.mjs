...
import { cleanMultiline, cleanText } from '../utils/text.mjs';
import {
...
function throwProviderError(status) {
  if (status === 401 || status === 403) {
    throw new AppError(503, 'AI_CONFIGURATION_ERROR', 'OpenRouter rejected the API key. Check OPENROUTER_API_KEY in your .env file.');
  }
  if (status === 402) {
    throw new AppError(503, 'AI_CREDITS_REQUIRED', 'OpenRouter has insufficient credits for the configured model.');
  }
  if (status === 404) {
    throw new AppError(503, 'AI_ROUTE_UNAVAILABLE', 'No compatible OpenRouter route is currently available for the model and privacy settings.');
  }
  if (status === 429) {
    throw new AppError(429, 'AI_RATE_LIMIT', 'OpenRouter is temporarily rate-limited. Wait a moment and try again.');
  }
  if (status === 400) {
    throw new AppError(502, 'AI_REQUEST_REJECTED', 'OpenRouter rejected the model request. Check OPENROUTER_MODEL in your .env file.');
  }
  throw new AppError(502, 'AI_SERVICE_ERROR', 'OpenRouter could not complete the request. Please try again.');
}

function conversation(history, message) {
  const messages = [];
  ...
  if (messages[0]?.role === 'assistant') {
    messages.unshift({ role: 'user', content: 'Use the following application-provided introduction as context.' });
  }
  append('user', cleanMultiline(message, 2400));
  return messages;
}

export function createAiService({ config, fetchImpl = globalThis.fetch, testMode = false }) {
  ...
  async function request({ messages, maxTokens, temperature, responseFormat }) {
    ...
    try {
      ...
      console.error('OpenRouter network request failed.', { name: error?.name || 'Error' });
      throw new AppError(502, 'AI_SERVICE_ERROR', 'The app could not reach OpenRouter. Check the network connection and try again.');
    } finally {
    ...
    if (choice?.error || choice?.finish_reason === 'error') throwProviderError(Number(choice?.error?.code || 502));
    if (choice?.finish_reason === 'length') {
      throw new AppError(502, 'AI_TRUNCATED_RESPONSE', 'The AI response was cut off. Please shorten the request and try again.');
    }

    const text = cleanMultiline(responseText(choice?.message?.content), 12_000);
    if (!text) throw new AppError(502, 'AI_EMPTY_RESPONSE', 'OpenRouter did not return a response. Please try again.');
    return text;
  }
  ...
  return {
    ...
    async analyze(user, payload) {
      ...
      if (testMode) return deterministicTestAnalysis(payload);
      ...
      if (analysis.conditions.length < 3) {
        throw new AppError(502, 'AI_INCOMPLETE_RESPONSE', 'The AI did not return enough distinct patterns. Add a little more symptom detail and try again.');
      }
      return analysis;
    },

    async chat(user, payload, recentCheck) {
      if (detectEmergency(payload.message)) return emergencyAnalysis().urgentMessage;
      if (testMode) return 'I can give general information, but I cannot diagnose you. What changed most recently?';
      ...
      return request({
        messages: [
          ...
          ...conversation(payload.history, payload.message)
        ],
        maxTokens: 500,
        temperature: 0.35
      });
